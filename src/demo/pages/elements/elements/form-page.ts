...
export default class FormPage extends LitElement {
	...
	static styles = [sharedStyles, css`
		...
		fieldset, wl-textfield, wl-textarea, wl-slider, wl-select {
			margin: 0 0 12px;
		}
	`];
				...
				<code-example-element>
					<form id="form">
						<fieldset>
							<legend>Title</legend>
							<wl-label>
								<wl-radio name="title" value="mr" checked></wl-radio>Mr.
							</wl-label>
							<wl-label>
								<wl-radio name="title" value="ms"></wl-radio>Ms.
							</wl-label>
						</fieldset>
						<wl-textfield name="age" required label="Enter your age" type="number" min="0" max="100" outlined @invalid="${this.validate}" @input="${this.resetValidation}"></wl-textfield>
						<wl-textfield name="fruit" required label="What's your favorite fruit?" pattern="[Bb]anana|[Aa]pple" outlined list="fruit-options" @invalid="${this.validate}" @input="${this.resetValidation}"></wl-textfield>
						...
						<wl-textfield name="email" type="email" required outlined label="What's your email?" @invalid="${this.validate}" @input="${this.resetValidation}"></wl-textfield>
						<wl-slider name="awesomeness" required outlined label="How awesome do you think this form is? (0% - 100%)" value="100" thumbLabel @invalid="${this.validate}" @input="${this.resetValidation}"></wl-slider>
						<wl-textarea name="message" outlined label="Leave a short message" style="--textarea-min-height: 100px; --textarea-max-height: 300px;"></wl-textarea>
